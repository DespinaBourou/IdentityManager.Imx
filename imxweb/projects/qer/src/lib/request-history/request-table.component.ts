...
//import { FilterWizardService } from 'qbm/lib/data-source-toolbar/filter-wizard/filter-wizard.service'; 
...
export class RequestTableComponent implements OnInit, OnDestroy, OnChanges { 
  ...
  public parameters: {objecttable: string; objectuid: string; filter?: FilterData[]};
  ...
  private readonly filterPresets = { ShowEndingSoon: undefined, ShowMyPending: undefined, status: undefined,MyDelegations:undefined }; 
  ...
  constructor( 
    ...
    this.subscriptions.push( 
      ...
      this.actionService.applied.subscribe(async () => { 
        ...
        console.log('INSIDE this.actionService.applied.subscribe'); 
  ...
  public async ngOnInit(): Promise<void> { 
    ...
    this.displayedColumns = [ 
      ...
      this.entitySchema.Columns.Email,
    ...
    try { 
      ...
      this.activatedRoute.queryParams.subscribe(async (params) =>{ //{ 
        ...
        if(params['source'] != 'tile' && this.source == 'tile'){ 
          ...
          this.dataToolbar.clearFilters(); 

        } 
        
        this.source = params['source']; 

        this.updateFiltersFromRouteParams(params) })

        console.log('FILTER PRESETS FROM INSIDE REQUEST TABLE IS', this.filterPresets); 
      ...
      this.groupData = createGroupData(
        this.dataModel,
        (parameters) => {
          return this.requestHistoryService.getGroupInfo({
            ...{
              PageSize: this.navigationState.PageSize,
              StartIndex: 0,
              objecttable: this.parameters?.objecttable,
              objectuid: this.parameters?.filter,
            },
            ...parameters,
          });
            }
      ...
      console.log('INSIDE REQUEST TABLE FILTER OPTIONS AND FILTER PRESETS ARE',this.filterOptions,this.filterPresets); 

      this.itShopConfig = (await this.projectConfig.getConfig()).ITShopConfig; 

      await this.getData(null, true);//}})//}}); 

    //puting everything inside subscribe will clear filters from datasource toolbar but not from table! 
...
//       filter = {...(groupedData.navigationState?.filter ?? []),...(this.parameters.filter ?? [])
...
//  }
  ...
  public updateFiltersFromRouteParams(params: Params): void { 
    ...
    console.log('FILTER PRESETS INSIDE UPDATE FROM ROUTE PARAMS', this.filterPresets); 
    ...
    this.navigationState.MyDelegations = this.filterPresets.MyDelegations; 

    this.navigationState.status = this.filterPresets.status; 

    // this.navigationState.filter.filter(option=>option.) 

    //this.dataToolbar.navigationChanged(this.navigationState) 
  ...
  public async getData(newState?: RequestHistoryLoadParameters, isInit = false): Promise<void> { 
    ...
    console.log('INSIDE GET DATA OF REQUEST TABLE COMPONENT NEW STATE IS',newState); 
    ...
    try { 
      ...
      if (data) { 
        ...
        console.log('REQUEST TABLE WILL SET DSTSETTINGS'); 
